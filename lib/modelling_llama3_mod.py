    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            if self.prune_method == "magnitude":
                ...
                module_contribution = self.down_proj.weight.data.to(torch.float32).pow_(2).mean(dim=0, keepdim=True)
                ins_ = intermed_result.view(-1, last_dim).to(torch.float32)
                ins_mean = ins_.mean(dim=0, keepdim=True) * module_contribution
                ins_sq = ins_.pow(2).mean(dim=0, keepdim=True) * module_contribution
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.prune_method == "magnitude":
            ...
            self.intermed_cache = (ins_mean, ins_sq)
        elif self.prune_method == "random": # LEFT UNEDITED
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        if self.prune_method == "magnitude":
            ...
            module_contribution = self.o_proj.weight.data.abs().to(torch.float32).pow_(2).mean(dim=0, keepdim=True)
            ins_ = attn_output.reshape(bsz, q_len, self.hidden_size).reshape(-1, self.hidden_size).to(torch.float32)
            ins_mean = ins_.mean(dim=0, keepdim=True) * module_contribution
            ins_sq = ins_.pow(2).mean(dim=0, keepdim=True) * module_contribution
            self.intermed_cache = (ins_mean, ins_sq)
