	...
	def forward(
		...
		if self.prune_method == "magnitude":
			...
			with torch.no_grad():
				...
				ins_ = ins_.pow(2).mean(0, keepdim=True).sqrt().to(self.o_proj.weight.dtype)
	...
	def forward(self, x):
		...
		with torch.no_grad():
			if self.prune_method == "magnitude":
				...
				ins_ = intermed_result.view(-1, last_dim).to(torch.float32).pow(2).mean(0, keepdim=True).sqrt().to(self.down_proj.weight.dtype)
				self.intermed_cache = (self.down_proj.weight.data.abs() * ins_).mean(axis=0).view(1, 1, -1)
...
# 	cache_dir = '/home/ldery/projects/PostHocModularization/my_wanda/llm_weights'
# 	from_pretrained = LlamaForCausalLM.from_pretrained(
...
# 		device_map="auto"
# 	)
# 	print('We just got here!')
	k, v = list(model.named_parameters())[0]
	print(k, v.mean().item(), v.max().item())
# 	diffs = []
# 	with torch.no_grad():
# 		for p1, p2 in zip(model.parameters(), from_pretrained.parameters()):
# 			diffs.append((p1 - p2.cpu()).sum().item())
# 	print(np.max(diffs))
# 	pdb.set_trace()