...
class PhiMLP(nn.Module):
	...
	def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
		...
		intermed_result = self.activation_fn(hidden_states)

		# Note that we have biases in this model !
		...
		with torch.no_grad():
			if self.prune_method == "magnitude":
				...
				ins_ = (self.fc2.weight.data.to(torch.float32).abs()) * (ins_.pow(2).mean(0, keepdim=True).sqrt())
		...
		hidden_states = self.fc2(intermed_result)
	...
	def forward(
		...
		if self.prune_method == "magnitude":
			...
			with torch.no_grad():
				...
				ins_ = (self.dense.weight.data.to(torch.float32).abs()) * ins_.pow(2).mean(0, keepdim=True).sqrt()
...
class PhiForCausalLM(PhiPreTrainedModel):
	...
	def __init__(self, config):
		...
		self.params_per_pruned_hidden = config.hidden_size * 2 # fc1 and fc2
