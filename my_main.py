...
def get_score_models(score_perfs, module_map, info_cache, hp_dict, wandb_run, all_sampling_proba, parent_id='.',  model_type='local'):
    ...
    for idx, truth_val in enumerate(is_valid):
        if truth_val:
            ...
            this_scores.append(ys[idx]) # TODO (LDERY) -- double check if this is better in the long run. Initial experiments said no 
...
def prune_attn(mask_, module):
    ...
    if mask_.mean() == 0: # We are pruning the whole module here !
        ...
        _, updated_kv_indices = find_pruneable_heads_and_indices(
                index, module.num_key_value_heads, module.head_dim, set()
        )
       
        new_k_proj = (prune_linear_layer(module.k_proj, updated_kv_indices)).half()
        module.k_proj = None
        module.k_proj = new_k_proj

        new_v_proj = (prune_linear_layer(module.v_proj, updated_kv_indices)).half()
        module.v_proj = None
        module.v_proj = new_v_proj

        qo_mask_ = repeat_kv(mask_.transpose(1, 2), module.num_key_value_groups).transpose(1, 2)
        index = (qo_mask_.squeeze() == 0).nonzero().squeeze()
        if index.numel() == 1:
            index = [index]
        ...
        module.num_key_value_heads = len(mask_.squeeze().nonzero())
