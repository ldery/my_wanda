...
def get_random_mask_scores(model, dataset, module_map, module_index_info, global_proba, use_complement=False, bsz=12, nsamples=32, mpi=100, pfrac=0.1, mlp_attn_ratio=1.0):
	# set to use main
	for k, (name, module) in module_map.items():
		module.is_using_main = False
...
def main():
	...
	print('Len Full datast = ', len(full_dataset))
	full_dataset, prior_dataset = full_dataset[:total_samples], full_dataset[total_samples:]
	print("Done loading prior and perturbation data : FullDs = {} | Prior Ds = {}".format(len(full_dataset), len(prior_dataset)))
